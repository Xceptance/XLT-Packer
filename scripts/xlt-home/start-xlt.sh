...
if [ $1 == "--help" ]; then
		...
		echo "  number-of-agents  :  How many agent controllers to start?"
		echo "  noremove          :  Remove old agent controller directories (if any)?"
        echo "Usage: $SCRIPTNAME <xlt-archive-file> <target-dir> <number-of-agents> [noremove]"
        exit 1
fi
...
if [ $# -lt 3 ]; then
        echo "Incorrect command line. Please specify the archive and the number of agents to install."
        exit 1
fi
...
PASSWORD=$(/var/local/getUserData.sh password)
...
for (( i = 0 ; i < $NUMBER_OF_AGENTS ; i++ )); do
        ...
        sed "s/com.xceptance.xlt.agentcontroller.port = 8500/com.xceptance.xlt.agentcontroller.port = $PORT/g" $AGENT_PREFIX$i/config/agentcontroller.properties > $AGENT_PREFIX$i/config/_agentcontroller.properties

        # Apply password
        if [ ! -z "$PASSWORD" ]; then
          sed "s/\(com\.xceptance\.xlt\.agentcontroller\.password\s*=\).*/\1 $PASSWORD/g" $AGENT_PREFIX$i/config/agentcontroller.properties > $AGENT_PREFIX$i/config/_agentcontroller.properties
        fi
        cp $AGENT_PREFIX$i/config/_agentcontroller.properties $AGENT_PREFIX$i/config/agentcontroller.properties
        rm $AGENT_PREFIX$i/config/_agentcontroller.properties
done
